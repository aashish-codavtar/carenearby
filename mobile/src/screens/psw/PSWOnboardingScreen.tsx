...
export function PSWOnboardingScreen() {
  ...
  async function submit() {
    ...
    try {
      ...
      // Mark onboarding complete in local storage so navigator knows to skip this screen
      if (user) {
        const updatedUser = { ...user, onboardingComplete: true };
        const token = await Storage.getToken();
        if (token) await Storage.saveAuth(token, updatedUser);
      }
      // Navigate to PSW dashboard (reset stack so user can't go back to onboarding)
        ...
        <View style={styles.pendingNotice}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.pendingDesc}>
              Your profile will be reviewed within 1–2 business days. You can browse the app
              while we verify your credentials.
            </Text>
          </View>
        </View>
